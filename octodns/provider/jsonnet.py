...
class JsonnetProvider(BaseProvider):
    '''
    Core provider for records configured in jsonnet files on disk.

    config:
        class: octodns.provider.jsonnet.JsonnetProvider
        # The location of jsonnet config files (required)
        directory: ./config
    ...
    def __init__(self, id, directory, default_ttl=3600, enforce_order=True,
                 *args, **kwargs):
        self.log = logging.getLogger('JsonnetProvider[{}]'.format(id))
        self.log.debug('__init__: id=%s, directory=%s, default_ttl=%d, '
                       'enforce_order=%d', id, directory, default_ttl,
                       enforce_order)
        super(JsonnetProvider, self).__init__(id, *args, **kwargs)
    ...
    def populate(self, zone, target=False, lenient=False):
        ...
        filename = join(self.directory, '{}jsonnet'.format(zone.name))
        file_data = json.loads(_jsonnet.evaluate_file(filename=filename))
        if file_data:
            for name, file_data in file_data.items():
